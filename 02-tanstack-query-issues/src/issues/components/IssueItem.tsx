...
export const IssueItem = ({issue}: Props) => {
  ...
  return (
    <div onMouseEnter={presetData} className="flex items-center px-2 py-3 mb-5 border rounded-md bg-slate-900 hover:bg-slate-800">
      ...
      <div className="flex flex-col flex-grow px-2">
        ...
        <div className='flex flex-wrap'>{issue.labels.map(label => (
          <span key={label.id} className='px-2 py-1 mx-2 text-xs text-white rounded-md' style={{border: `1px solid #${label.color}`}}>
            {label.name}
          </span>
        ))}</div>
